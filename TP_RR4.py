...
# ----- Flow definitions
def Inflow(m, j, t):
    """ Function for calculating inflows into a given tank. """
    if j == 'S':
        return m.RainIn[t] + sum(m.Overflow[j, t] for j in m.OFSTanks)
    elif j == 'D':
        return m.TotalDischarge['S', t] - m.AdjDischargeSH[t]
    elif j == 'H':
        return m.AdjDischargeSH[t] + m.Overflow['T', t]
    elif j == 'T':
...
def OverflowX1(m, j, t):
    """ Overflow equation definition. X2 = 0. Overflow = max{X1, 0}. """
...
def CondFlowX1(m, c, t):
    """ Conditional flow equation definition. X2 = 0. CondFlow = max{X1, 0}. """
    if c in ['SD2', 'SH']:
        source = 'S'
    elif c == 'DO2':
        source = 'D'
...
def TotalDischargeX1(m, j, t):
    """ X1 definition for adjusting discharge. TotalDischarge = min{X1, Outflow}. """
...
def RelativeSHVariable(m):
    """ A proportional method for calculating the adjusted SH rates. """
    return m.RelSHScalar == m.OrificeArea['SH'] / (m.OrificeArea['SD'] + m.OrificeArea['SD2'])

def AdjustSHFxMain(m, t):
    return m.AdjDischargeSH[t] == m.AdjSHDummy[1, t] + m.AdjSHDummy[2, t] - m.AdjSHDummy[3, t]
# -----
def AdjSHDummyBinFx1(m, t):
    return m.AdjSHBinary[t] <= m.CondBinary['SH', t]

def AdjSHDummyBinFx2(m, t):
    return m.AdjSHBinary[t] <= m.DischargeBinary['S', t]

def AdjSHDummyBinFx3(m, t):
    return m.AdjSHBinary[t] >= m.CondBinary['SH', t] + m.DischargeBinary['S', t] - 1
# -----
def AdjSHLogicFx1(m, n, t):
    if n == 2:
        Bin = m.CondBinary['SH', t]
        UB = m.DischargeLimit['SH']
    else:
        Bin = m.AdjSHBinary[t]
        UB = m.BigM

    return m.AdjSHDummy[n, t] <= Bin * UB

def AdjSHLogicFx2(m, n, t):
    if n == 1:
        Var = m.RelSHScalar * m.TotalDischarge['S', t]
    else:
        Var = m.Discharge['SH', t]

    return m.AdjSHDummy[n, t] <= Var

def AdjSHLogicFx3(m, n, t):
    if n == 1:
        Var = m.RelSHScalar * m.TotalDischarge['S', t]
        Bin = m.AdjSHBinary[t]
    elif n == 2:
    ...
    return m.AdjSHDummy[n, t] >= Var - (1 - Bin) * m.BigM
...
def CreateAbstractModel(DeltaT = 300, CatchmentSize = 15.7, Eps = 5000):
    ...
    m.n = pyo.Set(initialize = [1, 3], doc = 'SH adjustment dummy variables with b*b*v operation.')
    ...
    m.AdjSHBinary = pyo.Var(m.t, within = pyo.Binary)
    ...
    m.AdjDischargeSH = pyo.Var(m.t, within = pyo.NonNegativeReals)
    ...
    # Dummy variables
    m.AdjSHDummy = pyo.Var(m.r, m.t, within = pyo.NonNegativeReals)
    ...
    m.AdjustSHRateM = pyo.Constraint(m.t, rule = AdjustSHFxMain)
    m.AdjSHDB1 = pyo.Constraint(m.t, rule = AdjSHDummyBinFx1)
    m.AdjSHDB2 = pyo.Constraint(m.t, rule = AdjSHDummyBinFx2)
    m.AdjSHDB3 = pyo.Constraint(m.t, rule = AdjSHDummyBinFx3)
    m.AdjSHLogic1 = pyo.Constraint(m.r, m.t, rule = AdjSHLogicFx1)
    m.AdjSHLogic2 = pyo.Constraint(m.r, m.t, rule = AdjSHLogicFx2)
    m.AdjSHLogic3 = pyo.Constraint(m.r, m.t, rule = AdjSHLogicFx3)
